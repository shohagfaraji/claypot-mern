...
import { getAuthorRecipes } from '@/features/recipes/api/get-author-recipes';
import type { AuthorRecipeListData } from '@/features/recipes/types';

interface AuthorRecipeListState extends AuthorRecipeListData {
  requestKey: string | null;
  isLoading: boolean;
  error: string | null;
}

const initialState: AuthorRecipeListState = {
  requestKey: null,
  recipes: [],
...
export function useAuthorRecipes(queryString: string) {
  ...
  useEffect(() => {
    ...
    void getAuthorRecipes(request, queryString, controller.signal)
      ...
      .catch((error: unknown) => {
        ...
        setState({
          ...
          isLoading: false,
          error: error instanceof Error ? error.message : 'Your recipes could not be loaded.',
        });
      });
